...
namespace yakbas {
    ...
    std::unique_ptr<T> PrintCmdLineStateAndExtractPtr(
        ...
        } else if (cmdLineParserResultPair.second == ParserResultCode::CMD_ARGS_AND_FAILED) {
            LOG4CPLUS_INFO(logger, "Command line args found but parsing failed...");
            LOG4CPLUS_INFO(logger, "Application will run with defaults...");
            commandLineInfoPtr = std::make_unique<T>();
        } else if (cmdLineParserResultPair.second == ParserResultCode::CMD_ARGS_AND_SUCCESS) {
        ...
        } else if (cmdLineParserResultPair.second == ParserResultCode::NO_CMD_ARGS_AND_SUCCESS) {
            LOG4CPLUS_INFO(logger, "No command line args found and parsing succeeded...");
            LOG4CPLUS_INFO(logger, "Application will run with defaults...");
            commandLineInfoPtr = std::move(cmdLineParserResultPair.first);
        }

        if (commandLineInfoPtr == nullptr) {
            throw std::runtime_error("CommandLineInfo cannot be nullptr");
        }
